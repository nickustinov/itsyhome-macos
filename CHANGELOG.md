...
- Fix the battery indicator overlapping the mode buttons on expanded climate rows – radiators and other battery-powered thermostats drew their battery badge on top of the Cool/Heat/Auto buttons when the row was on. The expanding rows (thermostat, AC, Home Assistant climate) repositioned every row-1 element except the badge; it now follows the device name (#163, thanks @norbertpotocki)
...
- Air quality sensors – HomeKit air quality services (SONOFF air quality monitors, Eve Room and similar) were silently dropped and never appeared in the menu; an accessory whose only sensor was air quality vanished entirely. They now show as sensor rows with the reported quality level (Excellent to Poor, localized) plus the CO2 ppm or VOC µg/m³ reading the service carries, and the temperature and humidity readings such devices embed in the air quality service are picked up too (#154, thanks @je-ru)
- CO2 and CO sensors show the measured concentration – rows used to show only the binary CO2/Clear word; the ppm level that HAP requires on every such sensor was never read. The row now reads e.g. "Clear · 563 ppm" and updates live (#155, thanks @je-ru)
- Pro badge on "Record sensor history" – the Advanced pane toggle was just greyed out for free users with no hint why; it now carries the same PRO badge as other Pro features until Pro is active (#149, thanks @bondskin)
...
- Auto groups – similar devices are folded into one-click groups ("All lights", "All switches", "All blinds", "All ACs", "All fans", "All locks"), both across the whole home at the menu's top level and inside each room. A group appears once 2+ matching devices exist, opens as a submenu with a master toggle plus its member devices, and works on HomeKit and Home Assistant. Each top-level group is draggable anywhere in Settings → Home like any other section, room groups reorder among the room's devices, every group can be hidden with the eye icon, and any auto group can be pinned to the menu bar with the pin icon – its dropdown carries the same master toggle and member rows. On by default; turn the whole feature off in Settings → Advanced
...
- Live or snapshots – a "Live video in grid" toggle in Settings → Cameras switches the grid between live streams and the previous snapshot behaviour. Live tiles are HomeKit-only: in Home Assistant mode the grid keeps snapshot tiles with tap-to-stream (HA streams are negotiated per view over WebRTC/HLS and don't support an always-on grid), and the toggle is hidden
- Fix thermostat temperature skipping every other degree in Fahrenheit – the ± steppers moved the underlying Celsius value by 1° (= 1.8 °F), so 72 jumped to 70. Steps are now unit-aware: in Fahrenheit they move the displayed degree by exactly one. Applies to target temperature and both Auto-mode thresholds, on HomeKit and Home Assistant thermostats (#137, thanks @dsmecher and @LordZork)
- Fix Dock stuck hidden after quitting Itsyhome – Quit used to terminate the process synchronously from inside the status-bar menu's tracking session, leaving the system's menu-tracking state (and with it the Dock's auto-hide) wedged until restart or sleep. Quit now lets the menu close first and tears down the camera panel's window and event monitors before exiting (#141, thanks @LordZork)
- Fix iCloud sync never arriving on a second Mac – the only pull ran at startup before HomeKit had delivered any accessories, so it silently did nothing and was never retried; worse, the fresh Mac's empty lists could then upload and delete the first Mac's synced data before ever reading it. Sync now pulls once per home as soon as accessory data is ready and holds all uploads until that pull has happened, and the sync engine finally logs what it's doing (Console subsystem com.nickustinov.itsyhome, category CloudSync) (#111, thanks @tsmith8674 and @LordZork)
- Fix endless black spinner on sleepy battery doorbells (Aqara G4 and similar) – a stream start against a camera that is still waking can hang without HomeKit ever calling back. Stream negotiation now has a 10-second watchdog that tears the attempt down and retries once (the failed attempt is usually what woke the camera – the manual "close and reopen" that used to be the workaround), and if the retry also hangs the panel returns to the grid instead of spinning forever (#139, thanks @ray19826)
- Complete localization – all strings added since 2.6 (automations, HomeKit bridge, sensor states, sensor history, networks, webhooks, and the new camera grid settings) are now translated into all 11 languages, and the camera panel itself (Back/Close buttons, empty state) is localized for the first time
...
- Fully reorderable menu – Settings → Home is now one list containing every top-level section of the menu: Favourites, Groups, Scenes, each room, Other and Batteries, each collapsible with a device count on its header. Drag any section anywhere; the menu follows exactly. Sections without content (no favourites yet, no global groups, no battery devices, ...) are skipped in both the list and the menu, and appear at their saved position once they have content. Existing room orders are preserved, and the webhook API and cloud sync keep seeing the same room ordering as before
- Top-level dividers – every separator in the menu (including the ones under Favourites and Groups that used to be fixed) is now a real, user-managed divider shown in Settings → Home, exactly like the ones inside rooms: drag them anywhere, right-click a section header to add a divider above it, right-click a divider to remove it
...
- Sensor history (Pro, opt-in) – "Record sensor history" in Settings → Advanced keeps 30 days of readings per sensor, shown as a chart in each sensor's submenu: a line for temperature/humidity (thermostat and AC current temperature included), an on/off timeline for binary sensors, with hover readout and a 24h/7d/30d range toggle. Captured off the existing update path (no extra polling), stored locally per home, "Clear history" button included. Periods where nothing was recording (app closed, Mac asleep) show as gaps in the chart rather than a misleading flat line (#133, thanks @mikeburgh)
- Battery level via the webhook API – `/info/<room>/<device>` now includes `battery` (0–100) and `batteryLow` for battery-powered accessories, resolved the same way as the menu's battery badge on both HomeKit and Home Assistant (#132)
