//
//  HALightMenuItem.swift
//  macOSBridge
//
//  Menu item for Home Assistant lights with hs+color_temp mode switching
//  Shows RGB color circle + CT button for color temp control
//

import AppKit

class HALightMenuItem: NSMenuItem, CharacteristicUpdatable, CharacteristicRefreshable, ReachabilityUpdatableMenuItem, LocalChangeNotifiable {
    ...
    init(serviceData: ServiceData, bridge: Mac2iOS?) {
        ...
        // Color circle (shows current RGB color)
    ...
    private func updateUI() {
        ...
        // Position color circle
        let colorCircleX = buttonRightEdge - buttonSize
        colorCircle.frame.origin = CGPoint(x: colorCircleX, y: buttonY)

        // Position CT button
        let ctButtonX = colorCircleX - buttonSize - DS.Spacing.xs
        ctButton?.frame.origin = CGPoint(x: ctButtonX, y: buttonY)

        // Calculate label width based on visible controls
        var labelRightEdge = buttonRightEdge
        if showColorCircle {
            labelRightEdge = colorCircleX - DS.Spacing.xs
        }
        if showCtButton {
            labelRightEdge = ctButtonX - DS.Spacing.xs
        }
        nameLabel.frame.size.width = labelRightEdge - labelX
    ...
    private func updateColorCircle() {
        let color = NSColor(hue: hue / 360.0, saturation: saturation / 100.0, brightness: 1.0, alpha: 1.0)
